  ...
  <tr>
   ...
   <td><input type="submit" value="查询"/></td>
		...
		<tr>
			...
			<a href="viewStudentInfo?sid=${e.sid }">详细信息</a>&nbsp;&nbsp;┊&nbsp;
			<a href="deleteAdmin?id=${e.sid }" onclick="return confirm('确定删除该学生?');">删除</a> 		
			</td>
		</tr> 
		</c:forEach>
		...
		<font size="3" color="blue">
	  		...
	  		<a href="findStudent?page=1&keyword=${keyword }">『首页』</a>
  			<c:if test="${page == 1}">
				<a href="#">[上一页]</a>	
  			</c:if>
  			<c:if test="${page!=1 }">
  				<a href="findStudent?page=${page-1 }&keyword=${keyword}">[上一页]</a>
  			</c:if>
  			<c:if test="${page == totalPage }">
  				<a href="#">[下一页]</a>
  			</c:if>
  			<c:if test="${page != totalPage }">
  				<a href="findStudent?page=${page+1 }&keyword=${keyword}">[下一页]</a>
  			</c:if>
			<a href="findStudent?page=${totalPage }&keyword=${keyword}">『尾页』</a>
  		</font>
  		</center>
		</td>
